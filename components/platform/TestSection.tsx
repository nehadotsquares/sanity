...
export default function TestSection({ data }: TestSectionProps) {
	return (
		<section
		...
		>
			<div className="mx-auto max-w-7xl px-6">
				...
				<div className="mb-16 grid gap-8 lg:grid-cols-2">
					{/* Title */}
					<div>
						<h2 className="text-4xl lg:text-5xl">{data?.heading}</h2>
					</div>
					...
					{data?.cards?.map((card, index) => (
						<SwiperSlide className="!h-auto" key={index}>
							<div className="h-full flex flex-col rounded-2xl border border-gray-800 p-6 transition-all duration-300 hover:border-[#ccff00] hover:shadow-[20px_0_50px_10px_#ccff0033]">
								...
								{/* Description */}
								<p className="flex-1 text-gray-400">{card.description}</p>
							</div>
						</SwiperSlide>
					))}
				</Swiper>
